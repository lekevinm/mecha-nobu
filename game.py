...
class GameCommands:
	...
	async def r6(self, ctx, username):
 		...
 		embed.add_field(name="Matches", value = "Won: {} Lost: {} Played: {}".format(player.matches_won, player.matches_lost, player.matches_played), inline=False)
 		embed.add_field(name="KD Ratio", value = "KD: {} KDA: {} WL: {}".format(format(kdr, '.2f'), format(kdar, '.2f'), format(wlr, '.2f')), inline=False)
 		await ctx.send(embed=embed)
